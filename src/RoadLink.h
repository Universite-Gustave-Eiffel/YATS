...
public:
	...
	void setIDRoadLink(int i);
	int getIDRoadLink();

	// Display
	void DisplyMe(); /* TODO corriger faute */

	// Processing
	float convertMeterToPix(float f);
	float convertAngleToGap();
	...
	inline bool isEqualLigth(float x, float y)
	{
	  ...
	  return std::abs((double)(x - y)) <= epsilon * std::abs((double) x);
	}
